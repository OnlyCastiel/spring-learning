    ...
    public String parse(String text) {
        ...
        while (start > -1) {
            ...
            if (start > 0 && src[start - 1] == '\\') {
                //寻找{@code openToken}的条件分支一：这一个条件判断 是处理出现了{@code openToken}，但是这个{@code openToken}前面出现了转义字符
                ...
                //寻找{@code openToken}的条件分支二：下面的条件判断表示 出现了{@code openToken} 且 这个{@code openToken}前面没有转义字符的情况===
